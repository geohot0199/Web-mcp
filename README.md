...
- **Take the model with you.** **Export ↗** (header) or **Download STL ⤓** (canvas toolbar) opens the export dialog and saves the current scene locally, with the file attached to the chat as a card you can re-download anytime. Agent-initiated exports do the same thing through the approval flow — the STL downloads and the file lands in the conversation. Nothing is ever uploaded.
  ...
  - **Intersecting bodies: a note, never a block.** Interpenetrating or exactly duplicated forms are detected up front and surfaced as an informational note naming each pair — in the export dialog, the toast, the activity log, the chat file card, and the export result itself — while the download always proceeds. Orbit deliberately does not attempt a boolean union (fragile for the coplanar, grid-aligned contact this studio produces): every exported form stays its own closed shell. Exact contact (stacked forms, butt joints) is fine and notes nothing.
...
- **Keyless Orbit connection bridge** (`js/orbit-connection.js`). A dependency-free, credential-free client for Orbit's planning model. There is no API key concept in the code at all — no key input, no provider accounts, no auth headers. It connects in one click, tests the connection without persisting anything, and hands open-ended creative builds to the planning model while every proposal still passes the human approval flow. If the connection is unavailable, the deterministic planner takes over automatically.
...
# All STL export checks passed (solidity, overlap detection for the non-blocking note, Magics colour encoding, texture baking)
```
...
**Privacy model.** No backend. Share links and STL files are local artifacts — no Orbit server is involved. Local storage is a convenience, not a service — and it’s re-validated field by field on restore. If you opt into the Orbit connection for open-ended planning, your scene objects (geometry, positions, materials, colours, tags, constraints) and remembered design preferences are sent to the Orbit connection so the planning model can propose changes. No credential is ever involved: there is no API key to store, persist, or leak — the connection is just a tab-local on/off state, cleared on refresh/close or with **Wipe everything**.
